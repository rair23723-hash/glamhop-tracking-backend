...
function normalizeActivity(activity, courierFromTrack, shipmentTrack0, shippingAddress) {
  ...
  // Parse location into city, state, country if they are missing and location has commas
  ...
  if (!location && !city && !state && shippingAddress) {
    city = clean(shippingAddress.city);
    state = clean(shippingAddress.province || shippingAddress.state);
    country = clean(shippingAddress.country_code || shippingAddress.country);
  }

  // Fallback 2: Shiprocket destination details
  if (!location && !city && !state && shipmentTrack0) {
    city = clean(shipmentTrack0.delivered_to || shipmentTrack0.destination || shipmentTrack0.origin);
  }
  ...
  // If city/state/country are separate or extracted, combine them as City, State, Country
