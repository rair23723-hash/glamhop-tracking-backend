...
async function getTrackingByAWB(awb) {
  ...
  // (e.g. label generated, not yet picked up). Return gracefully with empty events.
  const trackStatus = Number(trackingData.track_status ?? 1);
  console.log(`[shiprocket] track_status: ${trackStatus}, activities count: ${activities.length}`);

  // Normalize events into a clean timeline array (most recent first)
  const events = activities.map((activity) => {
    // Shiprocket timestamps are always IST (UTC+5:30).
    // Append the offset so the browser Date constructor parses them correctly.
