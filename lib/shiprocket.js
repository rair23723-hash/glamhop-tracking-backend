...
async function authenticate() {
  ...
  if (!email || !password) {
    const missing = [!email && 'SHIPROCKET_EMAIL', !password && 'SHIPROCKET_PASSWORD'].filter(Boolean).join(', ');
    const err = new Error(`[shiprocket] Missing required env vars: ${missing}. Set them in Vercel Dashboard → Project Settings → Environment Variables.`);
    err.statusCode = 503;
    throw err;
  }
  ...
  try {
    ...
    // Don't re-wrap our own env-var error
    if (err.statusCode === 503) throw err;

    const status  = err?.response?.status;
    const message = err?.response?.data?.message || err.message;
    console.error(`[shiprocket] Authentication failed (HTTP ${status}):`, message);
    console.error('[shiprocket] Full auth error response:', JSON.stringify(err?.response?.data || '(none)'));
...
function normalizeActivity(activity, courierFromTrack) {
  ...
  const rawTime = (activity.ScanTime || activity.scan_time || '').trim();
  ...
  let city    = activity.city    || activity.City    || '';
  let state   = activity.state   || activity.State   || '';
  let country = activity.country || activity.Country || '';
  ...
  const srStatusLabel = activity['sr-status-label'] || activity.sr_status_label || '';
  const activityText  = activity.activity || activity.Activity || activity.ScanType || activity.description || '';
