...
async function getTrackingByAWB(awb) {
  ...
  // Log the raw Shiprocket response before any processing
  console.log('[shiprocket] 📦 Raw tracking_data received:');
  ...
  if (!trackingData) {
    // Shiprocket returned 200 but no tracking_data at all — AWB not in system
    const err = new Error('AWB not found in Shiprocket');
    err.statusCode = 404;
    throw err;
  }
  ...
  // Shiprocket uses different field names across courier integrations.
  // Try all known variants and pick the first non-empty array.
  const rawActivities =
    (Array.isArray(trackingData.shipment_track_activities) && trackingData.shipment_track_activities.length > 0
      ? trackingData.shipment_track_activities
      : null) ||
    (Array.isArray(trackingData.activities) && trackingData.activities.length > 0
      ? trackingData.activities
      : null) ||
    (Array.isArray(trackingData.shipment_activities) && trackingData.shipment_activities.length > 0
      ? trackingData.shipment_activities
      : null) ||
    [];

  console.log('[shiprocket] Activity field audit:');
  console.log(`  shipment_track_activities : ${JSON.stringify(trackingData.shipment_track_activities)?.slice(0, 120)}`);
  console.log(`  activities                : ${JSON.stringify(trackingData.activities)?.slice(0, 120)}`);
  console.log(`  shipment_activities       : ${JSON.stringify(trackingData.shipment_activities)?.slice(0, 120)}`);
  console.log(`  => Using ${rawActivities.length} event(s)`);

  // track_status === 0 means Shiprocket accepted the AWB but has no scan data yet
  const trackStatus = Number(trackingData.track_status ?? 1);
  console.log(`[shiprocket] track_status: ${trackStatus}, rawActivities: ${rawActivities.length}`);

  // Normalize events into a clean timeline array
  const events = rawActivities.map((activity) => {
    // Shiprocket date formats vary by courier:
    //   Format A: "2026-07-06 16:12:00"  (YYYY-MM-DD HH:mm:ss)
    //   Format B: "Jul 06 2026 04:12 PM" (Mon DD YYYY HH:MM AM/PM)
    // We convert both to a valid ISO 8601 string with +05:30 (IST) appended.
    let rawDate = (activity.date || '').trim();
    let isoDate = '';
    if (rawDate) {
      if (/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}/.test(rawDate)) {
        // Format A: replace the single space separator with T
        isoDate = rawDate.replace(' ', 'T');
        if (!isoDate.includes('+') && !isoDate.includes('Z')) isoDate += '+05:30';
      } else {
        // Format B or unknown: try to parse as-is and convert to IST ISO
        const parsed = new Date(rawDate);
        if (!isNaN(parsed)) {
          // Shift to IST by adding offset (raw date is assumed IST even if no zone)
          isoDate = rawDate; // keep as-is; browser Date() handles "Jul 06 2026 04:12 PM"
        } else {
          isoDate = rawDate; // pass through — will show as '—' in UI gracefully
        }
      }
    }
    ...
    const country  = activity.country || (city || state ? 'India' : '');
    const rawLoc   = activity.location || '';

    // If city/state are missing, try to parse them from the location string
    // e.g. "FAIZABAD-HUB" → location only; "Faizabad, Uttar Pradesh" → split
