...
async function getTrackingByAWB(awb, shippingAddress) {
  ...
  // ── Retrieve associated Order ID from Shiprocket details ────────────────────
  const orderId = shipmentTrack0?.order_id;
  let orderNumber = null;

  if (orderId) {
    try {
      console.log(`[shiprocket] Resolving Shiprocket order details for ID: ${orderId}`);
      const orderDetail = await shiprocketGet(`/orders/show/${orderId}`);
      orderNumber = orderDetail?.data?.channel_order_id || orderDetail?.data?.order_number || null;
      console.log(`[shiprocket] Resolved associated Shopify order number: "${orderNumber}"`);
    } catch (err) {
      console.warn(`[shiprocket] ⚠️ Failed to fetch order details for ID ${orderId}:`, err.message);
    }
  }
